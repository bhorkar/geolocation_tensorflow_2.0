...
#filelist = ['3_62263-.csv','']
window_size = 2
def worker(f):
    cutf = f.split('.csv')[0] + '_' + str(window_size)
    strcmd = "python main.py --filename {} --epoch 150 --model_output ./model_train/output_{} --preprocess 1 --serving_output_dir ./model_serve/model_{} --transform_dir ./tft_output/tft_train_data_{} --train_raw_data ../raw_data/ --batch_size 1024 --prediction_dir prediction_output_window_2> logs/out2_{} 2>logs/error2_{} ".format(
        f,
        cutf,
        cutf,
        cutf,
...
def test_run():
     futures = []
     with cf.ProcessPoolExecutor(10) as pp:
         for f1 in filelist:
